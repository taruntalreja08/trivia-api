...
def create_app(test_config=None):
    ...
    def delete_question(id):
        ...
        try:
            ...
            if question is None:
                abort(404)
            question.delete()
            return jsonify({"delete": id, "success": True}), 200
        except NotFound:
            abort(422)
    ...
    def get_questions_for_category(id):
        ...
        """
        if int(id) not in [category.id for category in Category.query.all()]:
            abort(404)
        return jsonify(
            {
                "total_questions": len(Question.query.filter(Question.category==id).all()),
                "questions": list(map(lambda question: question.format(),Question.query.filter_by(category=id))),
                "categories": {
                    category.id: category.type for category in Category.query.all()
                },
                "current_category": Category.query.filter(Category.id == id).one_or_none().type,
                "success": True
            }
    ...
    def get_quiz_question():
        """
        end point to "Play" tab and quiz
        """
        try:
            ...
            if int(quiz_category.get('id')):
                questions = Question.query.filter_by(category=str(quiz_category.get('id')))
            
            return jsonify(
                {
                    "question": questions.filter(Question.id.notin_(previous_questions)).first().format(),
                    "success": True
                }
            ), 200
